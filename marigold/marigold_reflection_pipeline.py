...
class MarigoldReflectionOutput(BaseOutput):
    """
    Output class for Marigold Reflection mask generation pipeline.

    Args:
        refl_mask (`np.ndarray`):
            Predicted reflection mask, with binary integer values in the range of [0, 1].
    """

    refl_mask: np.ndarray


class MarigoldReflectionPipeline(DiffusionPipeline):
    """
    Pipeline for Marigold Reflection Mask Generation pipeline.
    ...
    def __call__(
        ...
        return MarigoldReflectionOutput(
            refl_mask=final_pred,
        )
